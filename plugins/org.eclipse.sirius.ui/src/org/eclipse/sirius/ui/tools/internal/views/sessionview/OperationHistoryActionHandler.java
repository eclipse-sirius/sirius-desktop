...
public abstract class OperationHistoryActionHandler extends Action implements ActionFactory.IWorkbenchAction, IAdaptable {
    ...
    private class PartListener implements IPartListener {
        ...
        public void partClosed(IWorkbenchPart part) {
            ...
            } else if ((site instanceof MultiPageEditorSite) && (part.equals(((MultiPageEditorSite) site).getMultiPageEditor()))) {
                dispose();
            }
        }
    ...
    private class HistoryListener implements IOperationHistoryListener {
        public void historyNotification(final OperationHistoryEvent event) {
            ...
            case OperationHistoryEvent.REDONE:
                if (event.getOperation().hasContext(getUndoContext())) {
                    ...
                    display.asyncExec(new Runnable() {
                        public void run() {
                            if (pruning) {
                                 ...
                                 * https:/
                                 * /bugs.eclipse.org/bugs/show_bug.cgi?id=101215
                                 */
    ...
    protected DesignerSessionView designerSessionView;
     ...
     */
    OperationHistoryActionHandler(IWorkbenchPartSite site, DesignerSessionView sessionView) {
        // string will be reset inside action
        super(""); //$NON-NLS-1$
        this.site = site;
        designerSessionView = sessionView;
    ...
    public void update() {
        ...
        if (enabled) {
            ...
            // This should not happen, it is already check by should enabled,
            // but it avoid NPE in tests.
            String label = operation != null ? operation.getLabel() : "";
            tooltipText = NLS.bind(getTooltipString(), label);
            text = NLS.bind(getCommandString(), shortenText(label));
        } else {
             ...
             */
            if (getUndoContext() != null && pruning) {
                flush();
            }
        }
        setText(text);
    ...
    final boolean isInvalid() {
        return getUndoContext() == null || site == null;
    }

    /**
     * Get the undo context that should be used. Return the default
     * {@link IUndoContext} of the {@link IWorkspaceCommandStack} of the
     * Session's EditingDomain, else return
    ...
    final IUndoContext getUndoContext() {
        ...
        if (designerSessionView.getCurrentlySelectedSession() != null) {
            Session currentlySelectedSession = designerSessionView.getCurrentlySelectedSession();
            if (currentlySelectedSession.getTransactionalEditingDomain().getCommandStack() instanceof IWorkspaceCommandStack) {
                // We use the command stack to get the default Undo Context
                undoContext = ((IWorkspaceCommandStack) currentlySelectedSession.getTransactionalEditingDomain().getCommandStack()).getDefaultUndoContext();
            }
        }
        return undoContext;
    }
