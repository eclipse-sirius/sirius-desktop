...
class PolymorphicService implements IPolymorphicService {
    ...
    public boolean appliesTo(Object[] target) {
        ...
        return implementers.stream().anyMatch(svc -> svc.appliesTo(target1));
    }

    @Override
    public Object call(Object[] target) throws EvaluationException {
        final Object[] target1 = target;
        List<IMonomorphicService> candidates = implementers.stream().filter(svc -> svc.appliesTo(target1)).toList();
