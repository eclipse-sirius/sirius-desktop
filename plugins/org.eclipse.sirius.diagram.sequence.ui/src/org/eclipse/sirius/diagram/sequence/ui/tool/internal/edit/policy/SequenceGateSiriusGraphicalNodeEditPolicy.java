...
public class SequenceGateSiriusGraphicalNodeEditPolicy extends SiriusGraphicalNodeEditPolicy {
    ...
    protected Command getConnectionCreateCommand(CreateConnectionRequest request) {
        ...
        if (new RequestQuery(request).isSequenceMessageCreation() && request.getLocation() != null && org.eclipse.gef.RequestConstants.REQ_CONNECTION_START.equals(request.getType())) {
            ...
            if (getHost() instanceof GateEditPart gep) {
                if (gep.getGate().getMessage().some()) {
                    // Only one message is allowed on a Gate
    ...
    protected Command getConnectionCompleteCommand(CreateConnectionRequest request) {
        ...
        if (request instanceof CreateConnectionViewRequest) {
            ...
            if (!requestQuery.isSequenceMessageCreation()) {
                ...
                boolean safeTargetGate = true;
                if (targetEditPart instanceof GateEditPart gep) {
                    // Only one message is allowed on a Gate
                    safeTargetGate = !gep.getGate().getMessage().some();
                }

                if (firstClickLocation != null && secondClickLocation != null && safeTargetGate) {
