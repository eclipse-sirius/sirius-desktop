...
/**
 * Updates a progress monitor as bytes are read from the input stream. Also
 * starts a background thread to provide responsive cancellation on read().
 * 
 ...
 */
// Copied from org.eclipse.team.internal.core.streams.ProgressMonitorInputStream
// CHECKSTYLE:OFF
public class ProgressMonitorInputStream extends FilterInputStream {
     ...
     * @param bytesTotal
     *            the number of bytes to read in total (passed to
     *            updateMonitor())
     ...
     *             if the operation was interrupted before all of the bytes
     *             specified have been skipped, bytesTransferred will be zero
     * @throws IOException
     *             if an i/o error occurs
     */
     ...
     * @throws InterruptedIOException
     *             if the operation was interrupted before all of the bytes
     *             specified have been skipped, bytesTransferred may be non-zero
     * @throws IOException
     *             if an i/o error occurs
     */
    ...
    private void update(boolean now) {
        if (bytesRead >= nextUpdate || now) {
            ...
            if (nextUpdate != lastUpdate)
                updateMonitor(nextUpdate, monitor);
