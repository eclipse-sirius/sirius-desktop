...
This behavior is not user friendly. If the user has layout this edge to avoid other nodes for example, the move of the source or of the target of this edge moves all the layout, all the bendpoints, of this edge.
...
* Closest distance: If this feature is enabled on a edge, the evolution will change the current behavior. Ie the existing bendpoints (not visible with closest distance) will be lost.
...
Another approach will be to used a new ModelChangeTrigger added to the SessionEventBroker. With this approach, we have only one call after the move of the selected nodes. The computing will be made only with GMF data. This approach was rejected because it does not respect the GMF philosophy.
...
All existing tests should be verify to see if some of them depends on the old behavior. It this case, they should be adapted to the new behavior.
