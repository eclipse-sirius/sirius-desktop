...
To avoid the dirty status, the concept of a **transient layer** is created. The transient layer contains only decorations.
At save, the transient layer application is not persisted in the diagram and decorations are not persisted either.

A transient layer has the following features:
 * activeByDefault : if true, the layer is activated at each diagram opening.
 * optional : if true the layer can be deactivated and if false, the layer is always activated. In the latter case, activeByDefault is ignored.
...
This specification includes the fix of this bug. As expected behavior, in manual refresh, the decorations of a DDiagramElement should be correctly updated after a change on the semantic element and/or elements returned by associated elements expression.

When applying a layer, transient or not, in manual refresh, the diagram elements won't be refreshed if the layer contains only decorations (contrary to the current behavior). Only the decorations will be refreshed.
...
@TransientLayer@ is added to distinguish layers which application in diagram should be persisted or not.
The abstract class BasicLayer is created which is a super class of Layer and TransientLayer. It owns the attributes decorationDescriptionsSet and icon.
For now, it will only be possible to add decorations in that kind of layer.
@DiagramDescription.transientLayers@ and @DiagramExtensionDescription.transientLayers@ are added to add a transient layer.
...
Changes are done in diagram.ecore in the packages @diagram@ and @diagram/description@ and viewpoint.ecore in the packages @viewpoint@ and @viewpoint/description@.

Refer to "technical specification":#tech_spec to see the change
...
As it is defined in this specification, the concept of transient layer is defined par the specifier in the VSM.
...
The concept of transient layer in the VSM would not be needed any more.
