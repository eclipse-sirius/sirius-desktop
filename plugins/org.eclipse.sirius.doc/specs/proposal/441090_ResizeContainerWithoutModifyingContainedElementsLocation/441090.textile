h1. Increase size of a container without modifying location of contained elements

h2. Preamble

_Summary_: Resize a container to the left, upwards, or both, without modifying the location of contained elements.

|_. Version |_. Status |_. Date        |_. Authors |_. Changes |
| v0.1        | DRAFT    | 2014-08-05 | lredor    | Initial version. |
| v0.1        | PROPOSAL | 2014-08-14 | lredor    | Fix typo. |


_Relevant tickets_:
* "441090, Increase size of a container without modifying location of contained elements":https://bugs.eclipse.org/bugs/show_bug.cgi?id=441090

h2. Introduction

The goal of this feature is to be able to resize a container to the left, upwards, or both, without modifying the location of contained elements.
!images/containerResize.gif!
Currently, as you can see above, with the old behavior, the location of contained elements are relative to the container.
With the new behavior, the contained elements remain at the same location.

h2. Detailed Specification

The old behavior, the current one, will remain available with a specific shortcut. Shift, Ctrl and Alt is already used by other GMF behaviors during resize. So we must use a character key as shortcut. The chosen character shortcut key is *m* as in "move children".
