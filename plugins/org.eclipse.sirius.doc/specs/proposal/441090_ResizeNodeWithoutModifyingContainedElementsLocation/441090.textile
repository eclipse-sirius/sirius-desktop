...
The old behavior, the current one, will remain available with a specific shortcut. Shift, Ctrl and Alt is already used by other GMF behaviors during resize. So we must use a combination of "CTRL" + a character key as shortcut. The chosen character shortcut key is *r* as in "relative" (the position of the children would remain relative to the resized node).
