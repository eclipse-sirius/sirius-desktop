...
* At the Sirius query level, make sure you call only operations which are deterministic in the query language(s) you use. For example, the @nMinimize()@ standard service on the legacy query language is not. Unfortunately, this information is not often documented in the query languages semantics, so some trial and error experimentation is required for now.
...
* *@var:@* This interpreter can only do direct access to the value of a named variable. For example, instead of <code>[containerView/]</code> (using Acceleo) or @<%$containerView%>@ (using the legacy language), the equivalent using the specialized interpreter would be @var:containerView@. As a special case, the pseudo-variable @self@ is available to access the current element (the evaluation context): @var:self@ is equivalent to <code>[self/]</code> in Acceleo, @<%self%>@ and @<%current%>@ using the legacy language.
...
* *@service:@* This interpreter can be used to directly invoke a service method (i.e. a Java method that follows conventions for "service methods":#service_methods ) on the current element. Currently this interpreters only supports services with no parameters, which correspond to Java methods with a single argument, the model element on which to invoke the service. For example, assuming the service class "@FamilyServices@":#family_services has been correctly registered in the VSM, the expression @service:getFamiliesContainingParents@ (note the absence of parenthesis) will invoke the @getFamiliesContainingParents@ on the current element if it is an instance of @Family@.
** Intended limitation exists to be more efficient: The services should be in the same *bundle* as the VSM and should be deployed. If the services already exist in another bundle, you must declare a "proxy" in the bundle of the _VSM_.
...
Here is an example of a Java service taken from the @com.obeodesigner.sample.family.design@ example (you can obtain the project's sources using _New... > Example... > Obeo Designer > Family Designer_):
