...
* *@service:@* This interpreter can be used to directly invoke a service method (i.e. a Java method that follows conventions for "service methods":#service_methods ) on the current element. For example, assuming the service class "@FamilyServices@":#family_services has been correctly registered in the VSM, the expression @service:getFamiliesContainingParents@ (note the absence of parenthesis) will invoke the @getFamiliesContainingParents@ on the current element if it is an instance of @Family@.
