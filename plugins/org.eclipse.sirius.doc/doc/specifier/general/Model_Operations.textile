...
_Tip:_ Using a no-op _Change Context_ (with a browse expression like @[self/]@  which does not actually change the context) can be useful to group multiple operations into a single "block", especially in contexts where only one model operation is supported (e.g. inside a _Begin_ element).
