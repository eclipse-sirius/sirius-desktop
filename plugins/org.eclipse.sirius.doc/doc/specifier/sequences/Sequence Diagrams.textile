...
As this interaction container semantic element is the same as the diagram semantic root, the tools creating element directly on the diagram (like creating a new instance role) will only need to add this interaction container mapping as an extra mapping. The tool model operation stays the same as the semantic element is the same (the container variable in the tool model operation). Note that the containerView will be different though.
