...
The gate typically represents a port for message connecting to a sub-component. You can have a gate on a combined fragment, interaction use or interaction container. As messages are connected to MessageEnd model elements, it is the MessageEnd that has a reference to the Gate.

  !images/gate.png!

You can move a message connected to a gate along from its parent but you can't reconnect it to a different gate, nor move it to a different parent and reconnect the gate there. For instance, on the diagram below, it is not possible to move the message from the interaction use ref.1 to ref.2.

  !images/gate_move.png!
...
The Semantic Candidates Expression is an interpreted expression returning the semantic elements for which the mapping will be evaluated and then a graphical element will represent the semantic element on the diagram.
...
The properties _Oblique_ is specific to Basic Messages. It allows the message to be displayed as oblique with source.y < target.y.
...
The gate mapping is used for displaying a gate on a combined fragment, interaction use or interaction container.

We will define the gate mapping:

  !images/new_gate_mapping.png!

It is defined like an execution mapping .

  !images/gate_mapping_properties.png!
  
Note that there is no reconnect tool for GateMapping. Gates do not reconnect to other parents.

h2(#tools). Tools

h3. Java Services

To use a java service in an interpreted expression, service must be define like this in odesign:
...
h4. Gate Creation Tool

A Gate can be created using _Gate Creation Tool_. Its specification is similar to the _Execution Creation Tool_.

  !images/gate_creation_tool.png!

The following variables can be used from inside the tool definition:
* @variable@: the element where the tool was applied and that will contain the newly created gate
* @viewVariable@: the graphical element where the tool was applied
* @startingEndPredecessor@ and @finishingEndPredecessor@: represent in the global event ends list, the element preceding the new execution start and the element preceding the new execution finish. To get the corresponding semantic end element associated to event end: @startingEndPredecessor.semanticEnd@ or @finishingEndPredecessor.semanticEnd@.
...
A variable is available for the interpreted expression : $preTarget. This variable is the semantic element associated to graphical element that is currently hovered by the mouse.
