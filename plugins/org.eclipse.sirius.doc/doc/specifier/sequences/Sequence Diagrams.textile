...
The gate typically represents a port for message connecting to a sub-component. You can have a gate on a combined fragment, interaction use or interaction container. A gate is composed of three elements:
* _Gate start_: the start occurrence of the gate;
* _Gate_: the gate by itself;
* _Gate finish_: the finish occurrence of the gate.
...
Here is an example of 2 gates on an interaction use.

  !images/gate.png!
  
Note that gates do not reconnect to other parents.
