...
The example below shows three lifelines and four messages. Note that @m3@ is a _reflective_ message which is received by the same lifeline which sent it, and @oblique_4@ is an _oblique_ message which has a target coordinate with an y-value less than or equal to its source coordinate.. 
