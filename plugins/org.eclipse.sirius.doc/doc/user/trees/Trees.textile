...
h2. Overriding tree item height

The tree editor always specify a font height to use when displaying tree items. If this height does not render well on your machine for any reason, you can tell Sirius to use the font height of your environment.

To do so, go to "Window>Preferences". In the preference window: 
* select the Sirius Tree preferences and check the preference "Always use standard font size".
 !images/runtime_font_height_preference.png!
* reopen all the tree editors currently opened. The font height used is now your runtime one.
