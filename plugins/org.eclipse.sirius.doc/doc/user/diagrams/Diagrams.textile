...
* _Auto-scale_: if checked, the exported diagram will be scaled to the maximum possible size (limited by system-specific limitations) while keeping its aspect ratio. This can produce higher resolution images (but also larger files).
...
* _Auto-scale when exporting as image_: determines the default behavior when exporting diagrams programmatically, and the initial value of the corresponding checkbox in the _Export as image_ dialog.
