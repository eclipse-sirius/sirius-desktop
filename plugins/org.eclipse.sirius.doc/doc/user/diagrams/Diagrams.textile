...
* The edge is connected to border node and the border node have several incoming and outgoing edges.
...
** Known limitation: In case of multi-selection, the overlap is done in the current context of existing border nodes and not in the context of location of border nodes after each straighten edges.
* A straighten action will be disabled if the "edge centering":../../specifier/diagrams/Diagrams.html#edgecentering is activated on an edge.
...
This feature aims to move the group {edge, labels, ports} in a single â€œdrag likeâ€� operation when using the F3 shortcut and drag/move the edge. The F3 key has no effect if you attempt a reconnect by moving the first or last edge end.
