...
h3. Reset Diagram Origin

This action is available within the diagram context menu ("Reset Origin") or via "M1" + "&" shortcut.
The diagram bounds (the rectangle formed by the highest, the leftmost, the lowest and the rightest elements) can have a negative origin or can be shifted toward the bottom-right with a blank zone at the top-left.
This action aims to move all diagram elements so that the diagram retrieves its origin while keeping the element layout.
