...
Since Viewpoint 6.9, you have the possibility to customize the routing style of all new edges from the preferences. In the preference page _Viewpoint/Viewpoint Diagram/Connections_, you can enable _user specific default values_ and select a default routing style for all new edges.

!images/preferences_viewpoint_diagram_connections_VP6.9.png!
...
* <span id="synchronized_preference">_Synchronized mode for new diagrams:_</span> If checked, all newly created diagrams will initially be in "unsynchronized":#synchronized_diagram mode. You can still change the mode manually after the diagram is created.
