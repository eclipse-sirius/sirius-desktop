...
h2(#sirius6.1.5). Changes in Sirius 6.1.5

None.

h2(#sirius6.1.4). Changes in Sirius 6.1.4

h3. User-Visible Changes

* <span class="label label-success">Added</span> It is now possible to move labels on border of node, or border node, all around the node. Before, for label larger than the node, only centered location was authorized on North or South side. On this maintenance release of Sirius, this feature needs a "flag" to be activated. If you want to benefit of this feature, you need to add the VM arg "-Dorg.eclipse.sirius.experimental.labelOnBorderImprovement=true". This feature needs an automatic migration of representations file. This is why it is not activated by default on this maintenance release. 
...
The goal of this migration participant is to keep centered labels visually fixed compared to previous version (see "bugzilla #549887":https://bugs.eclipse.org/bugs/show_bug.cgi?id=549887 for more details). The corresponding version, stored in attribute version of viewpoint:DAnalysis of the aird file, is _14.1.4.201908131800_. As its associated feature, it is enabled only when VM arg @org.eclipse.sirius.experimental.labelOnBorderImprovement@ is set to @true@.

h2(#sirius6.1.3). Changes in Sirius 6.1.3

h3. Developer-Visible Changes

* <span class="label label-success">Added</span> The @viewpoint.ecore@ model has been added the an EMap<EJavaObject, EJavaObject> @subDiagramDecorationDescriptors@ in @UIState@ model element. It is used as a cache to speed up sub diagram decorations computing.
