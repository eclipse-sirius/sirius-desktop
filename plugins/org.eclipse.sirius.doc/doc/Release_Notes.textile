...
* <span class="label label-info">Modified</span> A "Link Note" is no longer deleted automatically when the targeted representation is deleted. Instead, the note header changes to "Broken Representation Link" and the note icon changes to a small red diagonal cross. You can refer to the "user documentation":user/diagrams/Diagrams.html#notes for more details.

h3. Developer-Visible Changes

h4. Migrations

* <span class="label label-success">Added</span> A migration participant has been added to update existing link notes. (see "bugzilla #533175":https://bugs.eclipse.org/bugs/show_bug.cgi?id=535648 for more details). The corresponding version, stored in attribute version of viewpoint:DAnalysis of the aird file, is _12.1.4.201810100944_.

h4. Changes in @org.eclipse.sirius.diagram.ui@

* <span class="label label-success">Added</span> The query @isLinkNote()@ has been added in @org.eclipse.sirius.diagram.ui.business.api.query.ViewQuery@ to know if the view is a link note or a normal note.
* <span class="label label-success">Added</span> The query @isLinkNoteBroken()@ has been added in @org.eclipse.sirius.diagram.ui.business.api.query.ViewQuery@ to know, for a link note, if it refers to a deleted representation descriptor. Invocations should be guarded by @isLinkNote()@.
