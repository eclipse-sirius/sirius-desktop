...
* <span class="label label-info">Modified</span> In Sirius table, default colors has been changed to use Preferences Settings and selection coloring behaves like other Views and Editors. Previously, line headers were forced to grey, selected cell was using Menu color instead of Theme color and no color was indicating the row on mouse hover.

h3. Specifier-Visible Changes

* <span class="label label-info">Modified</span> In tables, expressions and tools are injected with more variables. 'table', 'containerView' and 'line' elements are now accessible. With those elements, Specifier can access to @AnnotationEntry@ elements of @DTable@ instances. This modification allows following use-cases: 
** Create virtual lines: lines to group elements in tree with criteria based on attributes values or on user query.
** Have columns unrelated to root element: for example, columns targeting ECore features to have dynamic feature table.
