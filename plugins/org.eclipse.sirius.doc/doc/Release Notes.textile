...
* The @NodeMapping.getBestStyle()@ EOperation has been removed. It was not used anywhere.  Although its usage is highly discouraged, if you really need to  you can currently use the internal method @MappingHelper.getBestStyle()@ instead (use @SiriusPlugin.getDefault().getInterpreterRegistry().getInterpreter(semanticElement)@ to obtain the @IInterpreter@ this method need). As with all internal code, there is no guarantee this method will still exist in future versions.
