...
* To break the dependencies from Core to Diagram, several services have been added to the @DialectServices@, with default implementations in @AbstractRepresentationDialectServices@:
** @createTask(CommandContext, ModelAccessor, ModelOperation, Session, UICallBack)@: create a new task corresponding to the ModelOperation. Only needed for ModelOperation specific to one dialect.
...
* @org.eclipse.sirius.ui.business.api.dialect.DialectUIManager.canExport(DRepresentation, ExportFormat)@: New method returning true if at least one dialect is able to export this representation to this format, false otherwise.
