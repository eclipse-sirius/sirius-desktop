...
* To break the dependencies from Core to Diagram, a service has been added to the @DialectServices@. It has a default implementation in @AbstractRepresentationDialectServices@: @createTask(CommandContext, ModelAccessor, ModelOperation, Session, UICallBack)@: Create a new task corresponding to the ModelOperation. Only needed for ModelOperation specific to one dialect.
