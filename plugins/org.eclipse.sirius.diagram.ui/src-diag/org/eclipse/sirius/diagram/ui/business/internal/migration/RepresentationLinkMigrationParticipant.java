...
public class RepresentationLinkMigrationParticipant extends AbstractRepresentationsFileMigrationParticipant {
     ...
     */
    public static final Version MIGRATION_VERSION = new Version("12.1.4.201810100944"); //$NON-NLS-1$
    ...
    protected void postLoad(DAnalysis dAnalysis, Version loadedVersion) {
        if (loadedVersion.compareTo(MIGRATION_VERSION) < 0) {
                ...
                for (DRepresentation representation : new DViewQuery(view).getLoadedRepresentations()) {
                    if (representation instanceof DDiagram) {
                        ...
                        if (option.some() && migrateDiagram(option.get())) {
                            builder.append(MessageFormat.format(Messages.LinkNoteMigrationParticipant_entry, representation.getName())).append(System.lineSeparator());
                            isModified = true;
                        }
                    }
                }
            ...
            if (isModified) {
                ...
                builder.insert(0, Messages.LinkNoteMigrationParticipant_title);
