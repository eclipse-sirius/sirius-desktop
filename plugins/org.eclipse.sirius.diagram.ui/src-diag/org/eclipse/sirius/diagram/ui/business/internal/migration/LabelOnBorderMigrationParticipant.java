...
public class LabelOnBorderMigrationParticipant extends AbstractRepresentationsFileMigrationParticipant {
     ...
     */
    public static final Version MIGRATION_VERSION = new Version("14.1.4.201908131800"); //$NON-NLS-1$

    private boolean migrationOccured;

    @Override
    public Version getMigrationVersion() {
        if (Boolean.getBoolean(DBorderItemLocator.LABEL_ON_BORDER_IMPROVMENT)) {
            return MIGRATION_VERSION;
        } else {
            // Sirius "ignores" this migration participant so the last migration participant is returned
            return DeleteMultipleConnectorStyleMigrationParticipant.MIGRATION_VERSION;
        }
    }

    @Override
    protected void postLoad(DAnalysis dAnalysis, Version loadedVersion) {
        if (Boolean.getBoolean(DBorderItemLocator.LABEL_ON_BORDER_IMPROVMENT) && loadedVersion.compareTo(MIGRATION_VERSION) < 0) {
    ...
    private boolean checkAndFixGMFCoordinatesOfLabelOnBorder(Node node) {
        ...
        for (Object child : node.getChildren()) {
            if (child instanceof Node) {
                ...
                if (id == NotationViewIDs.DNODE_NAME_EDIT_PART_VISUAL_ID || id == NotationViewIDs.DNODE_NAME_2_EDIT_PART_VISUAL_ID || id == NotationViewIDs.DNODE_NAME_3_EDIT_PART_VISUAL_ID || id == NotationViewIDs.DNODE_NAME_4_EDIT_PART_VISUAL_ID) {
