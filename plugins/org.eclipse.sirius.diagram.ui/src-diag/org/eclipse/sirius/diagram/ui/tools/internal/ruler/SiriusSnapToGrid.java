...
public class SiriusSnapToGrid extends SnapToGridEx {
    ...
    private void ajustForFixedSizeNode(ChangeBoundsRequest request) {
        if (request.getEditParts().size() == 1 // on multi-selection,
                && request.getEditParts().get(0) instanceof ShapeNodeEditPart ep // nodes
                && ep.getModel() instanceof Node node && node.getLayoutConstraint() instanceof Size size) {

            // It is easier to shift the origin than to alterate the result
