...
public class PinnedElementsHandler {
    ...
    private boolean hasRemainingSolvableOverlaps() {
        for (IGraphicalEditPart part : fixedEditParts) {
            if (!Collections2.filter(findOverlappingParts(part), Predicate.not(isPinned)).isEmpty()) {
                return true;
            }
        }
        return false;
    ...
    private void packHorizontally() {
        ...
        final List<IGraphicalEditPart> movableParts = new ArrayList<IGraphicalEditPart>(Collections2.filter(allEditParts, Predicate.not(isPinned)));
    ...
    private void resolveOverlaps(final IGraphicalEditPart fixedPart) {
        final Set<IGraphicalEditPart> solvableOverlaps = Sets.filter(findOverlappingParts(fixedPart), Predicate.not(isPinned));
        ...
        assert Collections2.filter(findOverlappingParts(fixedPart), Predicate.not(isPinned)).isEmpty() : Messages.PinnedElementsHandler_remainOverlapsMsg;
    }
    ...
    private Map<IGraphicalEditPart, Point> moveAside(final Set<IGraphicalEditPart> parts, final Set<IGraphicalEditPart> fixedParts, final Direction dir,
        ...
        if (!overlaps.isEmpty()) {
            ...
            final Set<IGraphicalEditPart> movableOverlaps = new HashSet<IGraphicalEditPart>(Collections2.filter(overlaps, Predicate.not(isPinned)));
         ...
         */
        assert Sets.intersection(Sets.filter(findOverlappingParts(fixedParts), Predicate.not(isPinned)), parts).isEmpty();
