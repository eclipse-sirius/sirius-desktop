...
public abstract class AbstractCanonicalSynchronizer implements CanonicalSynchronizer {
     ...
     */
    public Option<View> getAncestor(View gmfView, int... visualID) {
        Option<View> result = Options.newNone();
        int type = SiriusVisualIDRegistry.getVisualID(gmfView.getType());
        for (int i = 0; i < visualID.length; i++) {
            if (type == visualID[i]) {
                result = Options.newSome(gmfView);
                break;
            }
        }
        if (!result.some() && gmfView.eContainer() instanceof View) {
            result = getAncestor((View) gmfView.eContainer(), visualID);
        }
        return result;
    }
