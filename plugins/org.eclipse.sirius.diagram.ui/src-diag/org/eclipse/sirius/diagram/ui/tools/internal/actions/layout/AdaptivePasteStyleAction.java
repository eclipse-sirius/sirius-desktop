 ...
 */
public class AdaptivePasteStyleAction extends Action implements IDisposableAction {
    ...
    // - Permission
    private IAuthorityListener onChangePermission = new IAuthorityListener() {
     ...
     * Default constructor.
     */
    public AdaptivePasteStyleAction() {
        super();

        setId(ActionIds.ADAPTIVE_PASTE_STYLE);
        setText(Messages.AdaptivePasteStyleAction_text);
        setToolTipText(Messages.AdaptivePasteStyleAction_toolTipText);
    ...
    private String getCommandLabel() {
        return Messages.AdaptivePasteStyleAction_commandLabel;
    }
    ...
    private Object getTargetEditPart(Object selection) {
        if (selection instanceof AbstractDEdgeNameEditPart editPart && editPart.getParent() instanceof AbstractDiagramEdgeEditPart) {
            ...
            for (IGraphicalEditPart targetEditPart : targetEditParts) {
                if (targetEditPart.resolveSemanticElement() instanceof DDiagramElement targetElement && canEditElement(targetElement)) {
                    ...
                    clipboard.getGmfView().ifPresent(gmfView -> {
                        command.add(PasteStyleCommandProvider.getGMFCommand(domain, targetEditPart, gmfView));
                    });
                    clipboard.getSiriusStyle().ifPresent(siriusStyle -> {
                        command.add(PasteStyleCommandProvider.getSiriusCommand(domain, targetElement, siriusStyle));
                    });
                }
            }
            if (!command.isEmpty()) {
