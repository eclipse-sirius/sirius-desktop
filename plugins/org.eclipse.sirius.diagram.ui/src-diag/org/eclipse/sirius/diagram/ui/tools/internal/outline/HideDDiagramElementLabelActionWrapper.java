...
/**
 * {@link IObjectActionDelegateWrapperWithImageDescription} dedicated to the contextual action "Hide label" of the
 * outline.
 * 
...
public class HideDDiagramElementLabelActionWrapper extends IObjectActionDelegateWrapperWithImageDescription {
     ...
     */
    public HideDDiagramElementLabelActionWrapper(IObjectActionDelegate action, String text) {
        super(action, text);
    }
    ...
    public boolean isEnabled() {
        ...
        if (currentSelection instanceof IStructuredSelection) {
            ...
            if (selectedObject instanceof DDiagramElement) {
            ...
            } else if (selectedObject instanceof AbstractDDiagramElementLabelItemProvider && ((AbstractDDiagramElementLabelItemProvider) selectedObject).getDiagramElementTarget().some()) {
                ...
                if (selectedObject instanceof DEdgeBeginLabelItemProvider) {
                    result = query.canHideLabel() && !query.isLabelHidden(DEdgeBeginNameEditPart.VISUAL_ID);
                } else if (selectedObject instanceof DEdgeLabelItemProvider) {
                    result = query.canHideLabel() && !query.isLabelHidden(DEdgeNameEditPart.VISUAL_ID);
                } else if (selectedObject instanceof DEdgeEndLabelItemProvider) {
                    result = query.canHideLabel() && !query.isLabelHidden(DEdgeEndNameEditPart.VISUAL_ID);
                } else {
                    result = query.canHideLabel() && !query.isLabelHidden();
                }
            }
