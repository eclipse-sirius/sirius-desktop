...
public class MinimizedTransparentCollapsingTest extends SiriusDiagramTestCase {
    ...
    private void checkDiagramConsistency(boolean collapsed) {
        ...
        List<DDiagramElement> borderNodes = new ArrayList<>(diagram.getDiagramElements().stream().filter(collapsable).toList());
    ...
    private void checkDiagramElements(boolean consistent) {
        ...
        assertEquals(consistent ? 24 : 0, Iterables.size(diagram.getDiagramElements().stream().filter(isVisible).toList()));
    }
