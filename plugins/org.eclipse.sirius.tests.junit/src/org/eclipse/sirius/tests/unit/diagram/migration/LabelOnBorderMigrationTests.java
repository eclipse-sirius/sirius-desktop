...
/**
 * A test ensuring that labels on border stay fixed after bugzilla 549887 that
 * allows more locations for label on border (all around the node). Before this
 * issue, the GMF coordinates does not correspond to the Draw2D displayed
 * coordinates. With this issue additional locations are authorized, so this
 * migration modifies the GMF coordinates to have the same displayed result than
 * before.
 * 
...
public class LabelOnBorderMigrationTests extends SiriusDiagramTestCase {
    ...
    /**
     * Check locations of labels in a diagram freshly created (without moving
     * labels) .
     * 
     * @throws Exception
    ...
    public void testLabelCoordinatesOfFreshlyCreatedDiagram() throws Exception {
        ...
        // Check that GMF coordinates (model) of label of border node have not
        // been changed (because it is not a label
        // to keep centered)
    ...
    /**
     * Check locations of labels in a diagram where labels have been manually
     * moved. The GMF locations are already and must remains the same.
     * <UL>
     * <LI>The label of node was centered on bottom of node. It must stay like
     * this (and GMF coordinates are already done).</LI>
     * <LI>The label of border node was centered on bottom of node. It must stay
     * like this (and GMF coordinates are already done).</LI>
     * </UL>
     * 
     * @throws Exception
     *             if unexpected issue occurs while performing this test
    ...
    public void testLabelCoordinatesOfDiagramWithMovedLabels() throws Exception {
        ...
        assertEquals("Wrong y coordinate for the label: The label must be on bottom of its node with one pixel between node and label.", nodeBounds.bottom() - IBorderItemOffsets.NO_OFFSET.height,
                nodeLabelBounds.y());
        // Check that GMF coordinates (model) of label of node have not been
        // changed
        Location gmfNodeLabelLocation = ((Location) ((Node) nodeLabelEditPart.getNotationView()).getLayoutConstraint());
    ...
    /**
     * Return the only item in the list. This method fails the test if the list
     * contains 0 or more than one element.
     * 
     * @param list
