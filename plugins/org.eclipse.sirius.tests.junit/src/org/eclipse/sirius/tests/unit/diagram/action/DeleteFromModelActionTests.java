...
public class DeleteFromModelActionTests extends SiriusDiagramTestCase implements EcoreModeler {
    ...
    public void testDeleteElementEdgeEditPart() throws Exception {
        ...
        List<DEdge> elementBasedEdges = new ArrayList<>(diagram.getEdges().stream().filter(relation).toList());
        ...
        List<DEdge> eltEdgesAfterDeleteTry = new ArrayList<>(diagram.getEdges().stream().filter(relation).toList());
