...
public class NoteShapeDefaultLabelAlignmentMigrationTest extends SiriusTestCase {
    ...
    public void testNoteShapeAlignmentAfterMigration() {
        ...
        UnmodifiableIterator<Shape> shapes = Iterators.filter(Iterators.filter(allContents, Shape.class), new Predicate<Shape>() {
            public boolean apply(Shape input) {
                return ViewType.NOTE.equals(input.getType());
            }
        });;
        if (shapes.hasNext()) {
            Shape note = shapes.next();
