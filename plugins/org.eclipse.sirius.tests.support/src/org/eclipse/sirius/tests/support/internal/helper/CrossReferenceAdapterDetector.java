...
public final class CrossReferenceAdapterDetector {
    ...
    public void checkNoCrossReferenceAdapter(ResourceSet resourceSet) {
        if (hasCrossReferenceAdapter(resourceSet)) {
            Resource res = Iterables.getFirst(Iterables.filter(resourceSet.getResources(), AirdResource.class), resourceSet.getResources().get(0));
            resourcesWithDetectedCrossReferenceAdapter.add(res.getURI());
        } else {
