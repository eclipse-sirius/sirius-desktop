...
public class SetMessageRangeOperation extends AbstractModelChangeOperation<Void> {
    ...
    public Void execute() {
        if (message.getElement() != null && message.getElement().eContainer() != null) {
            ...
            if (currentMessage.getElement() instanceof DEdge edge && source.getElement() instanceof EdgeTarget) {
                edge.setSourceNode((EdgeTarget) source.getElement());
            }
            currentMessage.setTarget(target);
            if (currentMessage.getElement() instanceof DEdge edge && target.getElement() instanceof EdgeTarget) {
                edge.setTargetNode((EdgeTarget) target.getElement());
            }
