...
/**
 * A section for the centeredTargetMappings property of a EdgeStyleDescription object.
 */
public class EdgeStyleDescriptionCenteredTargetMappingsPropertySection extends AbstractEditorDialogPropertySection {
    /**
    ...
    private boolean isActivated() {
        if (eObject instanceof EdgeStyleDescription) {
            ...
            return current != CenteringStyle.BOTH && current != CenteringStyle.TARGET;
        }
        return true;
    }
    ...
    private void removeUnrelatedMappings(List<?> choiceOfValues) {
        ...
        if (container instanceof EdgeMapping) {
            EList<DiagramElementMapping> targetMappingsList = ((EdgeMapping) container).getTargetMapping();
            choiceOfValues.retainAll(targetMappingsList);
        }
    }
